...
export async function getAgenda(req: Request, res: Response) {
  try {
    ...
    type SlotRow = {
      id: string; specialty_id: string; slot_time: Date
      max_capacity: number; used_capacity: number
  ...
  try {
    ...
    await Promise.all(days.map(async (day) => {
      ...
      if (day.is_closed) {
        ...
        // 2. UPSERT capacity rules for each hourly slot within the new range
        ...
        for (const cap of day.capacity_by_specialty) {
          ...
          const rows = slotTimes.map(
            (t) => Prisma.sql`(gen_random_uuid(), ${cap.specialty_id}::uuid, ${companyId}, ${day.day_of_week}, ${t + ':00'}::time, ${cap.max_capacity}, TRUE)`,
          )
          ...
          // Deactivate rules outside the new time range for this specialty+day
          await prisma.$executeRaw`
          ...
          // Update max_capacity on future slots without appointments (single query per specialty/day)
          await prisma.$executeRaw`
            UPDATE petshop_slots
